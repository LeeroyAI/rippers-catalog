...
struct TripPlannerView: View {
    ...
    private func searchNearbyRidingAreas(around coordinate: CLLocationCoordinate2D) async {
        ...
        async let r1 = fetchMapItems(query: "mountain bike trail", region: region)
        async let r2 = fetchMapItems(query: "bike park", region: region)
        async let r3 = fetchMapItems(query: "singletrack", region: region)
        let combined = await r1 + r2 + r3
        nearbyRidingAreas = sanitizeNearbyResults(combined, around: coordinate, maxDistanceKM: 120)
    }

    private func fetchMapItems(query: String, region: MKCoordinateRegion) async -> [MKMapItem] {
        let request = MKLocalSearch.Request()
